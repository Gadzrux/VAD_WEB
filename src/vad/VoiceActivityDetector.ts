...
export class VadDetector {
  ...
  constructor(
    ...
    if (!modelPath) {
      throw new Error("VAD_MODEL_PATH environment variable is not set");
    }
    ...
    this.reset();
    console.log(
      `VadDetector initialized with: startThreshold=${startThreshold}, endThreshold=${endThreshold}, samplingRate=${samplingRate}`
    );
  }
  ...
  async apply(
    ...
    try {
      ...
      if (
        speechProbPromise &&
        Array.isArray(speechProbPromise) &&
        speechProbPromise[0]
      ) {
