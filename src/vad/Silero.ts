...
class OnnxWrapper {
  ...
  private _last_sr: number = 0;
  private _last_batch_size: number = 0;
  ...
  async call(x: number[][], sr: number): Promise<number[][]> {
    // console.log(
    //   `Calling model with input shape: [${x.length}, ${x[0].length}], sample rate: ${sr}`
    // );
    await this.ready();
    ...
    if (sr === 8000 || sr === 16000) {
      ...
      //   console.log(
      //     `Model call completed, output shape: [${out.length}, ${out[0].length}]`
      //   );
      return out;
    } else {
      throw new Error(
  ...
  async audio_forward(x: number[][], sr: number): Promise<number[][]> {
    console.log(
      `Running audio_forward with input shape: [${x.length}, ${x[0].length}], sample rate: ${sr}`
    );
    const outs: number[][][] = [];
    [x, sr] = this._validate_input(x, sr);
    this.resetStates();
    ...
    console.log(
      `audio_forward completed, output shape: [${outs.length}, ${outs[0].length}]`
    );
    return outs.reduce((acc, curr) =>
      acc.map((row, i) => [...row, ...curr[i]])
    );
  }
